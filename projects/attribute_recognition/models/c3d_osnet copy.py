...
class C3D(nn.Module):
    def __init__(self, num_classes=487):
        ...
        self.cbam = CBAM3D(512)  # Thêm CBAM vào đây
    
    def forward(self, x):
        x = self.features(x)
        x = self.cbam(x)  # Áp dụng CBAM
        x = x.squeeze(2)  # Chuyển từ (B, C, D, H, W) -> (B, C, H, W)
        return x
...
osnet = OSNet(
    num_classes=1,
    blocks=[OSBlock, OSBlock, OSBlock],
    layers=[OSBlock, OSBlock, OSBlock],
    channels=[64, 256, 384, 512],
)

# Dữ liệu đầu vào giả lập
input_tensor = torch.randn(1, 3, 16, 128, 128)  # (B, C, D, H, W)

# Truyền qua C3D với CBAM rồi đến OSNet
c3d_output = c3d(input_tensor)
osnet_output = osnet(c3d_output)

print(osnet_output.shape)
